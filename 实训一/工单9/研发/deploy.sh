#!/bin/bash
# ============================================================
# 智能文档问答系统（RAG + 知识图谱 + 语音 + RAGAS评估） - 一键部署
...
update_config() {
    ...
    # 更新 ragas_eval.py 中的路径和密钥
    sed -i 's|SAMPLE_PDF = r"C:\\Users\\.*|SAMPLE_PDF = "'"${UPLOAD_DIR}"'/sample_questions.pdf"|g' "${APP_DIR}/ragas_eval.py"
    sed -i 's|EMBEDDING_MODEL_PATH = r"C:\\Users\\.*|EMBEDDING_MODEL_PATH = "'"${EMBEDDING_MODEL}"'"|g' "${APP_DIR}/ragas_eval.py"
    sed -i "s|MIMO_API_KEY = .*|MIMO_API_KEY = \"${MIMO_API_KEY}\"|g" "${APP_DIR}/ragas_eval.py"
    sed -i "s|MIMO_BASE_URL = .*|MIMO_BASE_URL = \"${MIMO_BASE_URL}\"|g" "${APP_DIR}/ragas_eval.py"
    cat > "${APP_DIR}/.env" << EOF
...
# ======================== Step 6: Streamlit 配置 ========================
setup_streamlit() {
    log_step "Step 6/9: 配置 Streamlit"
...
[Unit]
Description=智能文档问答系统（RAG + 知识图谱 + RAGAS评估）
...
# ======================== Step 9: 运行 RAGAS 评估 ========================
run_ragas_eval() {
    log_step "Step 9/9: 运行 RAGAS 评估"
    source ${VENV_DIR}/bin/activate
    cd ${APP_DIR}
    if [[ -f "ragas_eval.py" ]]; then
        log_info "开始 RAGAS 评估（Context Precision + Recall）..."
        python ragas_eval.py 2>&1 | tee ${LOG_DIR}/ragas_eval.log
        if [[ -f "ragas_report.json" ]]; then
            local precision recall
            precision=$(python3 -c "import json; r=json.load(open('ragas_report.json')); print(r['summary']['context_precision'])")
            recall=$(python3 -c "import json; r=json.load(open('ragas_report.json')); print(r['summary']['context_recall'])")
            log_info "评估结果: Precision=${precision}, Recall=${recall}"
        fi
    else
        log_warn "ragas_eval.py 不存在，跳过评估"
    fi
    deactivate
}
...
print_summary() {
    ...
    echo -e "${GREEN}║${NC}    📊 RAGAS 评估（Context Precision + Recall）               ${GREEN}║${NC}"
    echo -e "${GREEN}╚══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
}

...
check_root() {
    [[ $EUID -ne 0 ]] && { log_error "请用 root 运行: sudo ./deploy.sh"; exit 1; }
}
...
setup_user_and_dirs() {
    ...
    log_info "目录: ${APP_DIR} | 数据: ${DATA_DIR} | 图片: ${IMAGE_DIR}"
}
...
[Unit]
Description=RAG 问答系统（多PDF+图片理解） - API
...
setup_firewall_and_start() {
    ...
    sleep 3
    systemctl is-active --quiet ${SERVICE_NAME} && log_info "✅ API 服务运行正常" || \
        log_warn "检查: journalctl -u ${SERVICE_NAME} -f"
}
...
verify_services() {
    ...
    if curl -sf http://localhost:${PORT}/api/health >/dev/null 2>&1; then
        ...
        log_warn "API 健康检查失败，可能还在初始化"
    fi
    # 图片服务检查
    if curl -sf http://localhost:${PORT}/api/image-status >/dev/null 2>&1; then
        local img_status
        img_status=$(curl -s http://localhost:${PORT}/api/image-status 2>/dev/null)
        log_info "图片服务: ${img_status}"
    fi
}
...
print_summary() {
    ...
    echo ""
    echo -e "${GREEN}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║   🚀 部署完成！多PDF + 图片理解智能问答系统已启动            ║${NC}"
    echo -e "${GREEN}╠══════════════════════════════════════════════════════════════╣${NC}"
    ...
    echo -e "${GREEN}╠══════════════════════════════════════════════════════════════╣${NC}"
    echo -e "${GREEN}║${NC}  API 接口:                                                      ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}    POST /api/chat            文本问答                           ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}    POST /api/analyze-image   图片分析                           ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}    POST /api/image-chat      图片+文档问答                      ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}    GET  /api/image-status    图片服务状态                       ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}    GET  /api/pdfs            PDF列表                            ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}    POST /api/upload-pdf      上传PDF                            ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}                                                              ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}  ⚠️  请确保已配置 .env 中的 API Key:                          ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}    - DEEPSEEK_API_KEY (文本问答)                              ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}    - QWEN_VL_API_KEY   (图片理解)                              ${GREEN}║${NC}"
    echo -e "${GREEN}╚══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
main() {
    echo "============================================================"
    echo "  招股说明书智能问答系统（多PDF+图片理解） - 一键部署"
    echo "============================================================"

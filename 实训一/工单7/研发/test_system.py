...
def test_config():
    ...
    print("=" * 60)
    try:
        from config import CHUNK_SIZE, DEEPSEEK_API_KEY, MAX_TOKENS, PDF_FILES, TEMPERATURE, TOP_K
...
def test_image_processor():
    ...
    try:
        ...
        if not processor.available:
            print("⚠️  测试跳过: 未配置 Qwen-VL API Key")
            return None

        print(f"模型: {processor.model}")
        print(f"API Base: {processor.api_base}")
        print("✓ 图片处理模块初始化成功")
        return True
    except Exception as exc:
        print(f"✗ 图片处理模块测试失败: {exc}")
        return False

...
# 2. 告诉 huggingface_hub 库，别提醒我符号链接的问题了
...
class RetrievalModeRequest(BaseModel):
    mode: str = Field(..., pattern="^(vector|bm25|hybrid)$")
...
async def get_retrieval_mode():
    ...
    return {
        ...
        "available_modes": ["vector", "bm25", "hybrid"],
    }
...
async def set_retrieval_mode(body: RetrievalModeRequest):
    """切换检索模式，触发索引重建。"""
    try:
        qa = get_qa_system(force_rebuild=True, retrieval_mode=body.mode)
        return {
            "status": "ok",
            "mode": body.mode,
            "message": f"已切换到 {body.mode} 检索模式，索引已重建",
        }
    except Exception as exc:
...
#  docker已经部署成功啦，在docker中启动 gongdan10-docker即可  直接就可以访问http://localhost:8080
#!/bin/bash
# ============================================================
# 招股说明书智能问答系统（Redis + Milvus + LightRAG 版） - 一键部署
...
# ======================== Step 2: Redis ========================
install_redis() {
    log_step "Step 2/11: 安装 Redis"
    if command -v redis-server &>/dev/null; then
        log_info "Redis 已安装: $(redis-server --version 2>&1 | head -1)"
    else
        case $OS in
            ubuntu|debian) apt-get install -y -qq redis-server ;;
            centos|rhel|rocky|almalinux) yum install -y redis || dnf install -y redis ;;
        esac
        systemctl enable redis; systemctl start redis
        log_info "Redis 安装完成"
    fi
    # 验证
    redis-cli ping 2>/dev/null && log_info "✅ Redis 运行正常" || log_warn "Redis 未启动"
}

# ======================== Step 3: Milvus (Docker) ========================
install_milvus() {
    log_step "Step 3/11: 安装 Milvus（Docker）"
    if ! command -v docker &>/dev/null; then
        log_info "安装 Docker..."
        case $OS in
            ubuntu|debian) apt-get install -y -qq docker.io ;;
            centos|rhel|rocky|almalinux) yum install -y docker || dnf install -y docker ;;
        esac
    ...
    if docker ps --format '{{.Names}}' | grep -q "milvus-standalone"; then
        log_info "Milvus 已在运行"
    else
        log_info "启动 Milvus Standalone..."
        # 下载启动脚本
        if [[ ! -f /opt/milvus/embedded-etcd.sh ]]; then
            mkdir -p /opt/milvus
            docker run -d --name milvus-etcd \
                -v /opt/milvus/etcd:/etcd \
                -e ETCD_AUTO_COMPACTION_MODE=revision \
                -e ETCD_AUTO_COMPACTION_RETENTION=1000 \
                -e ETCD_QUOTA_BACKEND_BYTES=4294967296 \
                milvusdb/etcd:latest 2>/dev/null || true

            docker run -d --name milvus-minio \
                -v /opt/milvus/minio:/minio_data \
                -e MINIO_ACCESS_KEY=minioadmin \
                -e MINIO_SECRET_KEY=minioadmin \
                minio/minio server /minio_data 2>/dev/null || true
            ...
            docker run -d --name milvus-standalone \
                ...
                milvusdb/milvus:latest 2>/dev/null || {
                    log_warn "Milvus 启动失败，将使用 FAISS 作为备选"
                    return
                }
        fi
        sleep 5
        docker ps --format '{{.Names}}' | grep -q "milvus" && log_info "✅ Milvus 运行正常" || log_warn "Milvus 启动失败"
    fi
}

# ======================== Step 4-8: 应用部署 ========================
setup_user_and_dirs() {
    log_step "Step 4/11: 创建用户和目录"
...
setup_env() {
    log_step "Step 7/11: 配置环境变量"
    cat > "${APP_DIR}/.env" << EOF
# DeepSeek API（双 Key 分流）
DEEPSEEK_API_KEY=***
DEEPSEEK_API_KEY_2=***
...
# ======================== Step 9: Systemd ========================
setup_systemd() {
    log_step "Step 9/11: 配置 Systemd 服务"
    cat > /etc/systemd/system/${SERVICE_NAME}.service << EOF
[Unit]
Description=RAG 问答系统（Redis + Milvus + LightRAG） - API
...
setup_firewall_and_start() {
    log_step "Step 10/11: 防火墙并启动"
    if command -v ufw &>/dev/null; then
        ufw allow ${PORT}/tcp 2>/dev/null || true
        ufw allow ${STREAMLIT_PORT}/tcp 2>/dev/null || true
        ufw allow 6379/tcp 2>/dev/null || true
        ufw allow 19530/tcp 2>/dev/null || true
        ...
        firewall-cmd --permanent --add-port=19530/tcp 2>/dev/null || true
...
# ======================== Step 11: 验证 ========================
verify_services() {
    log_step "Step 11/11: 验证服务"
    curl -sf http://localhost:${PORT}/api/health >/dev/null 2>&1 && log_info "✅ 健康检查通过" || log_warn "API 还在初始化"
    redis-cli ping 2>/dev/null && log_info "✅ Redis 正常" || log_warn "Redis 不可用"
    log_info "Milvus: curl http://localhost:${MILVUS_HOST}:9091/healthz 2>/dev/null || log_warn 'Milvus 不可用'"
}
...
print_summary() {
    ...
    echo ""
    echo -e "${GREEN}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║   🚀 部署完成！智能问答系统已启动                             ║${NC}"
    echo -e "${GREEN}╠══════════════════════════════════════════════════════════════╣${NC}"
    ...
    echo -e "${GREEN}║${NC}  📦 Redis:       ${REDIS_HOST}:${REDIS_PORT}                    ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}  🗄️  Milvus:      ${MILVUS_HOST}:${MILVUS_PORT}                  ${GREEN}║${NC}"
    echo -e "${GREEN}╠══════════════════════════════════════════════════════════════╣${NC}"
    echo -e "${GREEN}║${NC}  核心能力:                                                      ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}    🔍 四种检索: vector / bm25 / hybrid / lightrag             ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}    📦 Redis 缓存: 查询缓存 + Embedding 缓存 + 会话管理       ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}    🗄️  Milvus: 向量存储 + 对话记忆检索                       ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}    🕸️  LightRAG: 知识图谱双层检索                             ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}    🖼️  图片理解: Qwen-VL 多模态                               ${GREEN}║${NC}"
    echo -e "${GREEN}╚══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
main() {
    echo "============================================================"
    echo "  招股说明书智能问答系统（Redis + Milvus + LightRAG） - 一键部署"
    echo "============================================================"
    check_root; detect_os
    install_system_deps; install_redis; install_milvus
    setup_user_and_dirs; deploy_app; setup_venv; setup_env
    update_config_paths; setup_systemd; setup_firewall_and_start; verify_services
    print_summary
}
main "$@"

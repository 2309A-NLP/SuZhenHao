...
MILVUS_METRIC_TYPE = os.getenv("MILVUS_METRIC_TYPE", "IP")  # IP=内积, L2=欧氏距离
MILVUS_NLIST = int(os.getenv("MILVUS_NLIST", 128))  # 聚类中心数

# 对话记忆配置
MEMORY_COLLECTION_NAME = os.getenv("MEMORY_COLLECTION_NAME", "conversation_memory")
MEMORY_TOP_K = int(os.getenv("MEMORY_TOP_K", 3))  # 对话记忆检索数量
MEMORY_SIMILARITY_THRESHOLD = float(os.getenv("MEMORY_SIMILARITY_THRESHOLD", 0.5))  # 对话记忆相似度阈值
...
MAX_HISTORY = int(os.getenv("MAX_HISTORY", 20))  # 每个会话最大消息数
CACHE_TTL = int(os.getenv("CACHE_TTL", 3600))  # 查询缓存过期时间（秒）

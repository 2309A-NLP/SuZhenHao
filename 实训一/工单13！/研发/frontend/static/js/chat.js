...
let useRedisSession = false; // 是否使用 Redis 存储会话

// 检查 Redis 是否可用
async function checkRedisSession() {
  try {
    const res = await fetch("/api/redis-status");
    const data = await res.json();
    useRedisSession = data.connected === true;
    if (useRedisSession) {
      console.log("[会话] 使用 Redis 存储");
      // 加载远程会话列表
      await loadSessionsFromRedis();
    } else {
      console.log("[会话] 使用 localStorage");
    }
  } catch (e) {
    console.log("[会话] Redis 不可用，使用 localStorage");
  }
}
...
function saveChat(title, messages) {
  ...
  // 同步到 Redis
  if (useRedisSession && currentChatId) {
    fetch("/api/sessions", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ chat_id: currentChatId, messages: messages }),
    }).catch((e) => console.warn("保存到 Redis 失败:", e));
  }
...
async function loadChat(id) {
  ...
  // 如果本地没有消息，从 Redis 加载
  if (useRedisSession && (!chat || !chat.messages || chat.messages.length === 0)) {
    try {
      const res = await fetch(`/api/sessions/${id}`);
      const data = await res.json();
      if (data.messages && data.messages.length > 0) {
        if (!chat) {
          chat = { id, title: "加载中...", messages: [], updatedAt: Date.now() };
          chatHistory.unshift(chat);
        }
        ...
        for (const msg of data.messages) {
          if (msg.role === "user") {
            chat.title = msg.text ? msg.text.substring(0, 40) : "新对话";
            break;
          }
        }
      }
    } catch (e) {
      console.warn("从 Redis 加载会话失败:", e);
    }
  }
...
clearHistoryBtn.addEventListener("click", async () => {
  ...
  if (useRedisSession) {
    ...
    } catch (e) { console.warn("清空 Redis 会话失败:", e); }
  }
  showToast(t("toastHistoryCleared"));
});

function deleteChat(id) {
  if (!confirm(currentLang === "zh" ? "确定删除这条对话记录？" : "Delete this conversation?")) return;
  chatHistory = chatHistory.filter((c) => c.id !== id);
  localStorage.setItem("chatHistory", JSON.stringify(chatHistory));
  // 删除 Redis 会话
  if (useRedisSession) {
    fetch(`/api/sessions/${id}`, { method: "DELETE" }).catch((e) => console.warn("删除 Redis 会话失败:", e));
  }
  if (currentChatId === id) {
    currentChatId = null;
    chatArea.innerHTML = "";
  ...
  renderHistory();
  showToast(currentLang === "zh" ? "已删除对话记录" : "Conversation deleted");
}
...
function finalizeStreamingMessage(group, bubbleEl, metaEl, sourcesContainer, data) {
  ...
  metaEl.innerHTML = `<span class="confidence-badge"><span class="confidence-dot ${confClass}"></span>${t("confidence")} ${(conf * 100).toFixed(1)}%</span>${data.retrieval_time != null ? ` <span class="retrieval-time-badge">⏱ ${t("retrievalTime")} ${data.retrieval_time}s</span>` : ""}`;
      ...
      <div class="message-row">
        ...
        <div class="message-body">
          ...
          <div class="message-meta">
            ...
            ${data.retrieval_time != null ? `<span class="retrieval-time-badge">⏱ ${t("retrievalTime")} ${data.retrieval_time}s</span>` : ""}
          </div>
          ${sourcesHtml}

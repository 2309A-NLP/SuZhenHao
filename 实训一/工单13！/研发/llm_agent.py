...
class RAGGenerator:
    ...
    def _build_messages(self, query: str, context: str, lang: str = "zh", history: list = None, memory_context: str = "") -> list:
        """构建消息列表（支持多轮对话）。"""
        messages = []
...
请给出简洁、准确的答案。"""
    ...
    def generate_answer(self, query: str, retrieved_docs: List[Dict], lang: str = "zh", history: list = None, memory_context: str = "") -> Dict:
        context = self._build_context(retrieved_docs, lang)

        if self.llm.available:
            try:
                messages = self._build_messages(query, context, lang, history, memory_context)
    ...
    def generate_answer_stream(self, query: str, retrieved_docs: List[Dict], lang: str = "zh", history: list = None, memory_context: str = ""):
...
class QASystem:
    ...
    def __init__(self, retriever, query_processor=None, conversation_memory=None):
        self.retriever = retriever
        self.query_processor = query_processor
        self.conversation_memory = conversation_memory
        self.generator = RAGGenerator()

    def _search_memory(self, query: str, lang: str = "zh") -> str:
        """检索对话记忆，返回格式化的记忆上下文。"""
        if self.conversation_memory is None or not self.conversation_memory.available:
            return ""
        try:
            memories = self.conversation_memory.search(query)
            if not memories:
                return ""
            parts = []
            for m in memories:
                ...
                parts.append(f"[{ts}] (相关度 {sim:.0%})\n{content}")
            header = "历史对话记忆：" if lang == "zh" else "Conversation Memory:"
            print(f"  [记忆] 找到 {len(memories)} 条相关记忆")
            return header + "\n" + "\n---\n".join(parts)
        except Exception as exc:
            print(f"⚠️  记忆检索失败: {exc}")
            return ""
    ...
    def _store_memory(self, query: str, answer: str):
        ...
        if self.conversation_memory is None:
            return
        try:
            self.conversation_memory.add(query, answer)
        except Exception as exc:
            print(f"⚠️  记忆存储失败: {exc}")
    ...
    def answer(self, query: str, lang: str = "zh", history: list = None) -> Dict:
        try:
            ...
            # 检索对话记忆
            memory_context = self._search_memory(query, lang)

            print("[检索中] 搜索相关文档...")
            t0 = time.time()
            retrieved_docs = self.retriever.retrieve(query)
            retrieval_time = round(time.time() - t0, 2)
            print(f"[检索完成] 找到 {len(retrieved_docs)} 个相关段落，耗时 {retrieval_time}s")
            print("[生成中] LLM 正在生成答案...")
            result = self.generator.generate_answer(query, retrieved_docs, lang, history=history, memory_context=memory_context)
            result["retrieval_time"] = retrieval_time

            # 存储对话记忆
            self._store_memory(query, result["answer"])
    ...
    def answer_stream(self, query: str, lang: str = "zh", history: list = None):
        ...
        try:
            ...
            metadata, content_gen = self.generator.generate_answer_stream(query, retrieved_docs, lang, history=history, memory_context=memory_context)
            ...
            def _capture_and_store():
                ...
                self._store_memory(query, full_answer)

            return metadata, _capture_and_store()
        except Exception as exc:
            error_msg = f"Error processing question: {exc}" if lang == "en" else f"处理问题时出错: {exc}"
            metadata = {"query": query, "sources": [], "confidence": 0.0, "retrieval_time": 0.0}

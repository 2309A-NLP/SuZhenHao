#!/bin/bash
# ============================================================
# 招股说明书智能问答系统（多PDF+图片理解+多轮对话+流式输出） - 一键部署脚本
...
setup_firewall_and_start() {
    ...
    systemctl is-active --quiet ${SERVICE_NAME} && log_info "✅ API 服务运行正常" || log_warn "检查: journalctl -u ${SERVICE_NAME} -f"
}

# ======================== Step 9: 验证 ========================
verify_services() {
    log_step "Step 9/9: 验证服务"
    curl -sf http://localhost:${PORT}/api/health >/dev/null 2>&1 && log_info "✅ API 健康检查通过" || log_warn "API 还在初始化"
...
print_summary() {
    ...
    echo ""
    echo -e "${GREEN}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║   🚀 部署完成！多模态智能问答系统已启动                       ║${NC}"
    echo -e "${GREEN}╠══════════════════════════════════════════════════════════════╣${NC}"
    ...
    echo -e "${GREEN}║${NC}    POST /api/chat            普通问答（支持多轮）               ${GREEN}║${NC}"
    ...
    echo -e "${GREEN}║${NC}    POST /api/upload-pdf      上传 PDF                           ${GREEN}║${NC}"
